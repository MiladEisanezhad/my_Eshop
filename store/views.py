...
from .models import (
    Banner, Brand, Category, Coupon, NewsletterSubscriber,
    Order, OrderItem, Product, Review, UserProfile, Wishlist,
)
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            ...
            'new_arrivals': (
                Product.objects
                .filter(status='published', is_new_arrival=True)
                .select_related('category')[:8]
            ),
            'on_sale_products': (
                Product.objects
                .filter(status='published', is_on_sale=True)
                .select_related('category')[:8]
            ),
            'banners': Banner.objects.filter(is_active=True, position='main_slider').order_by('ordering'),
            'category_banners': Banner.objects.filter(is_active=True, position='category_banner').order_by('ordering')[
                                :3],
...
class ShopView(ListView):
    ...
    def get_queryset(self):
        qs = Product.objects.filter(status='published').select_related('category', 'brand')
        p = self.request.GET
        ...
        if category_slug:
            self._selected_category = get_object_or_404(Category, slug=category_slug, is_active=True)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            ...
            'price_range': Product.objects.filter(status='published').aggregate(min_p=Min('price'), max_p=Max('price')),
...
class ProductDetailView(View):
    ...
    def _build_context(self, product):
        ...
        return {
            ...
            'variants': variants,
            'sizes': list(variants.values_list('size', flat=True).distinct().exclude(size='')),
            'colors': list(variants.values_list('color', 'color_hex').distinct().exclude(color='')),
            'reviews': product.reviews.filter(is_approved=True).select_related('user'),
            'related_products': Product.objects.filter(category=product.category, status='published').exclude(
                id=product.id)[:4],
            'variants_json': json.dumps([
                {'id': v.id, 'size': v.size, 'color': v.color, 'stock': v.stock, 'price': str(v.final_price)}
                for v in variants
            ]),
        }
...
class CheckoutView(View):
    ...
    def get(self, request):
        ...
        ctx = self._build_context(request, cart, cart_items, coupon_obj, coupon_discount)
        return render(request, self.template_name, ctx)
...
class SearchSuggestionsView(View):
    def get(self, request):
        ...
        if len(q) >= 2:
            products = (
                Product.objects
                .filter(name__icontains=q, status='published')
